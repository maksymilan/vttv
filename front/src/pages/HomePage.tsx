...
export const HomePage: React.FC = () => {
  ...
  const handleSendClick = async (e?: React.MouseEvent | React.KeyboardEvent) => {
    e?.stopPropagation();
    console.log("📤 发送动作触发 - selectedFile:", selectedFile?.name, "currentMessage:", currentMessage.slice(0, 30));
    
    if (!selectedFile && !currentMessage.trim()) {
       console.log("⚠️ 没有文件也没有消息，取消发送");
       return;
    }
    ...
    if (selectedFile && currentSessionId) {
        console.log("🎬 开始上传视频流程:", selectedFile.name);
