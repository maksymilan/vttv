import React from 'react';

import styles from './index.module.scss';

const Component = () => {
  return (
    <div className={styles.frame}>
      <div className={styles.autoWrapper6}>
        <div className={styles.group1}>
          <div className={styles.frame1}>
            <p className={styles.text}>基础版</p>
            <p className={styles.text2}>手机版</p>
            <p className={styles.text3}>联系我们</p>
            <div className={styles.frame3}>
              <p className={styles.english}>English</p>
              <div className={styles.rectangle13} />
              <div className={styles.rectangle14} />
              <div className={styles.rectangle15} />
            </div>
          </div>
          <div className={styles.autoWrapper2}>
            <div className={styles.autoWrapper}>
              <div className={styles.ellipse4} />
              <div className={styles.ellipse5} />
              <div className={styles.ellipse3} />
              <div className={styles.ellipse8} />
              <div className={styles.ellipse2}>
                <div className={styles.ellipse6} />
              </div>
              <div className={styles.ellipse7} />
              <div className={styles.ellipse9} />
              <div className={styles.ellipse11} />
              <div className={styles.ellipse10} />
            </div>
            <div className={styles.instance}>
              <p className={styles.a1}>
                上传你的康复视频，结合专业标签要求，精准解析动作细节。
              </p>
            </div>
          </div>
          <div className={styles.rectangle7} />
          <div className={styles.rectangle6}>
            <p className={styles.text4}>登录</p>
          </div>
        </div>
        <p className={styles.linksEveryMoveToBett}>
          Links every
          <br />
          move to better healing.
        </p>
        <div className={styles.rectangle2}>
          <div className={styles.autoWrapper3}>
            <p className={styles.text5}>验证码登录</p>
            <p className={styles.text6}>密码登录</p>
          </div>
          <div className={styles.rectangle12} />
          <p className={styles.text9}>
            <span className={styles.text7}>
              注册登录即代表已阅读并同意我们的&nbsp;
            </span>
            <span className={styles.text8}>用户协议</span>
            <span className={styles.text7}>&nbsp;与&nbsp;</span>
            <span className={styles.text8}>隐私政策</span>
          </p>
          <div className={styles.group11}>
            <div className={styles.rectangle8} />
            <p className={styles.text10}>请输入手机号</p>
            <p className={styles.a86}>+86</p>
            <img src="../image/mjvr5oxx-glv3t2d.svg" className={styles.vector} />
          </div>
          <div className={styles.group12}>
            <div className={styles.autoWrapper4}>
              <div className={styles.rectangle8} />
              <img src="../image/mjvr5oxx-ho8rv0l.svg" className={styles.vector2} />
              <img src="../image/mjvr5oxx-6893klb.svg" className={styles.vector3} />
            </div>
            <div className={styles.autoWrapper5}>
              <p className={styles.text11}>请输入密码</p>
              <p className={styles.text12}>忘记密码？</p>
            </div>
          </div>
          <p className={styles.text13}>没有账号？去注册</p>
          <div className={styles.rectangle62}>
            <p className={styles.text14}>登录</p>
          </div>
        </div>
      </div>
      <div className={styles.autoWrapper8}>
        <div className={styles.autoWrapper7}>
          <div className={styles.group8}>
            <img src="../image/mjvr5oxx-oywvpzq.svg" className={styles.group5} />
            <p className={styles.healink}>Healink</p>
          </div>
          <p className={styles.text15}>注册</p>
        </div>
        <p className={styles.text16}>在这里，AI 联结每一个动作，助力高效康复</p>
        <p className={styles.text17}>游客访问不会保存数据</p>
        <div className={styles.group16}>
          <p className={styles.text6}>直接开始康复评估</p>
          <img src="../image/mjvr5oxx-idlu9e2.svg" className={styles.vector4} />
        </div>
      </div>
    </div>
  );
}

export default Component;
